...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    private static final RowMapper<Meal> RAW_MAPPER = (rs, i) -> {
    ...
    public List<Meal> getAll(int userId) {
        return jdbcTemplate.query("SELECT *  FROM meals WHERE user_id=? ORDER BY datetime DESC", RAW_MAPPER, userId);
    }
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE user_id=? AND datetime BETWEEN ? AND ? ORDER BY datetime DESC",
                RAW_MAPPER, userId, startDate, endDate);
    }
}
